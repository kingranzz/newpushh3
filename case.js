...
if (!m.quoted) return m.reply(`*Contoh penggunaan :*
ketik ${cmd} m.reply pesan viewoncenya`)
let msg = m?.quoted?.message?.imageMessage || m?.quoted?.message?.videoMessage || m?.quoted?.message?.audioMessage || m?.quoted
if (!msg.viewOnce && m.quoted.mtype !== "viewOnceMessageV2" && !msg.viewOnce) return m.reply("Pesan itu bukan viewonce!")
...
case "brat1": {
if (!text) return
const media = `https://lyyn-api.my.id/imagecreator/brat?text=${encodeURIComponent(text)}`
...
case "pushkontak4": case "puskontak4": {
if (!isOwner) return m.reply(mess.owner)
if (!text || !text.includes("|")) return m.reply(`Masukan pesan & nama kontak\n*Contoh :* ${cmd} pesan|namakontak`)
global.textpushkontak = text.split("|")[0]
let rows = []
...
case "pushkontak3": case "puskontak3": {
if (!isOwner) return m.reply(mess.owner)
if (!text || !text.includes("|")) return m.reply(`Masukan pesan & nama kontak\n*Contoh :* ${cmd} pesan|namakontak`)
global.textpushkontak = text.split("|")[0]
let rows = []
...
await sock.sendMessage(m.chat, {
  ...
  headerType: 1,
  viewOnce: true,
  text: `\nPilih Target Grup PushkontakV3\n`
}, { quoted: m })
}
break
...
case "pushkontak5": case "puskontak5": {
if (!isOwner) return m.reply(mess.owner)
if (!text || !text.includes("|")) return m.reply(`Masukan pesan & nama kontak\n*Contoh :* ${cmd} pesan|namakontak`)
global.textpushkontak = text.split("|")[0]
let rows = []
...
case "1bbgb": case "2gbbbb": case "3gbbb": case "4gbbb": case "bb5gb": 
case "6hhhgb": case "7hhgb": case "8gbbb": case "9ghhb": case "bb10gb": 
case "unlimibbted": case "unnnli": {
